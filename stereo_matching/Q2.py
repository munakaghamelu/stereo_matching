...
#FORWARD PASS
from toGrayScale import getImg
...
def create_stereoGram():
    print("yoooo")
    size_Big = np.random.rand(512, 512)
    Image.fromarray((size_Big * 255).astype('uint8'), mode='L').save('A.png')
    A = Image.open('A.png')
    size_Small = np.random.rand(256, 256)
    Image.fromarray((size_Small * 255).astype('uint8'), mode='L').save('B.png')
    B = Image.open('B.png')
    #L = A.paste(B, (124,128)).save('L.png')
    A.paste(B, (124,128))
    A.save('L.png')
    A = Image.open('A.png')
    A.paste(B, (132, 128))
    ...
    try:
        with open('disparity_array.txt', 'w') as f:
            for item in disparity_map:
                f.write("%s\n" % item)
        Image.fromarray((disparity_map * 255).astype(np.uint8), mode='L').save('disparityMap.png')
    except:
        print("Unable to save")
    finally:
        try:
            array = disparity_map * 255
            fig = plt.figure()
            plt.imsave('disparityMath.png', array.astype('uint8'), cmap=matplotlib.cm.gray, vmin=0, vmax=255)
            plt.close(fig)
        except:
            print("Unable to use plt")
        finally:
            Image.fromarray((disparity_map * 255).astype('uint8'), mode='L').save('disparityMap_2.png')
